...
parser.add_argument('--embedding_module', type=str, default="diffusion", choices=["diffusion"], help='Zebra diffusion embedding')
...
for i in range(args.n_runs):
  ...
  # Ignore the first (Numba compilation) measurement when later epochs exist.
  measured_tppr_times = train_tppr_time[1:] if len(train_tppr_time) > 1 else train_tppr_time
  NUM_EPOCH=stop_epoch if stop_epoch!=-1 else NUM_EPOCH
  logger.info(f'### num_epoch {NUM_EPOCH}, epoch_train {t_total_epoch_train/NUM_EPOCH}, epoch_val {t_total_epoch_val/NUM_EPOCH}, epoch_test {t_test}, train_tppr {np.mean(measured_tppr_times)}')
  ...
  if not args.save_best:
    ...
    if args.snapshot_count is not None:
      Path(best_checkpoint_path + '.split.json').unlink(missing_ok=True)
