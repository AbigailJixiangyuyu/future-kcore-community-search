...
class TGN(torch.nn.Module):
  def __init__(self, neighbor_finder, node_features, edge_features, device, n_layers=2,
               ...
               memory_dimension=100, embedding_module_type="diffusion",
  ...
  def compute_node_embeddings_readonly(self, node_ids, timestamps):
    """Return temporal node embeddings without mutating memory or T-PPR."""
    if self.embedding_module_type != 'diffusion':
      raise ValueError('Read-only node encoding currently requires diffusion')
    ...
    return self.embedding_module.compute_embedding_tppr_ensemble_readonly(
      memory, node_ids, timestamps
    )

  def decode_edge_probabilities(self, source_embeddings,
                                destination_embeddings):
    """Decode directed edge probabilities from precomputed embeddings."""
    if source_embeddings.shape != destination_embeddings.shape:
      raise ValueError('source and destination embedding shapes must match')
