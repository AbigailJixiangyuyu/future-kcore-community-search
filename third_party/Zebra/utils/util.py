...
class tppr_finder:
  ...
  def query_topk(self, source_nodes, timestamps):
    """Extract current T-PPR neighborhoods without updating maintained state."""
    n_nodes = len(source_nodes)
    batch_node_list = []
    batch_edge_idxs_list = []
    batch_delta_time_list = []
    ...
    for tppr_index in range(self.n_tppr):
      PPR_list = self.PPR_list[tppr_index]
      for position in range(n_nodes):
        node = source_nodes[position]
        timestamp = timestamps[position]
        self.extract_streaming_tppr(
          PPR_list[node], timestamp, self.k,
          batch_node_list[tppr_index],
          batch_edge_idxs_list[tppr_index],
          batch_delta_time_list[tppr_index],
          batch_weight_list[tppr_index],
          position,
        )
