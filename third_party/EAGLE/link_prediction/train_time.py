...
from utils.util import set_random_seed, compute_metrics, print_model_info, EarlyStopMonitor, NegEdgeSampler, get_device, cached_negatives
...
parser.add_argument("--split-times", type=int, nargs=2, default=None,
                    metavar=("TRAIN_END_TS", "VAL_END_TS"),
                    help="inclusive snapshot timestamps for train and validation")
...
parser.add_argument(
    ...
    help="Train/select on train/validation data and save weights; skip test negatives and scoring.",
)
...
full_data, train_data, val_data, test_data, n_nodes, n_edges = get_data_transductive(
    DATA, use_validation=True, data_path=args.data_path, split_times=args.split_times
)
...
if not args.train_only:
    ...
    # Preserve the initialization seed normally set by the test sampler.
...
def process_data(mode, finder, data, bs, num_neg=1, filepath=None):
    ...
    for batch_idx in tqdm(range(0, num_batch)):
        ...
        neg_filepath = f"./data/batchneg/{DATA}/{mode}_neg{num_neg}_bs{bs}_batch{batch_idx}.pkl"
        sampler = {"Train": train_neg_edge_sampler, "Val": val_neg_edge_sampler,
                   "Test": test_neg_edge_sampler}[mode]
        negatives_batch = cached_negatives(neg_filepath, sampler, destinations_batch).t().flatten()
        ...
        all_inds = torch.tensor(all_inds, dtype=torch.long, device=device)
...
# Epoch exhaustion must use the same best weights as early stopping.
...
if args.train_only:
    print(
        f"Training complete: best_epoch={early_stopper.best_epoch}, "
        f"checkpoint={best_model_path}; test scoring skipped.",
        flush=True,
    )
    raise SystemExit(0)
