...
def get_device(gpu):
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA unavailable; pass --gpu -1 for CPU.")
    return torch.device(f"cuda:{gpu}")


def cached_negatives(path, sampler, destinations):
    """Persist [positive event, negative sample] order shared by both branches."""
    path = Path(path)
    if path.exists():
        with path.open("rb") as handle:
            samples = torch.as_tensor(pickle.load(handle)).cpu()
        if samples.ndim != 2 or tuple(samples.shape) != (len(destinations), sampler.num_neg):
            raise ValueError(f"Incompatible negative cache: {path}. Use a fresh working directory.")
    else:
        samples = sampler.sample(destinations).cpu()
...
class NegEdgeSampler:
    def __init__(self,  destinations: np.ndarray, full_destinations: np.ndarray, num_neg: int, device: str = 'cuda', seed: int = 2024):
        ...
        if not 1 <= num_neg < len(self.unique_destinations):
            raise ValueError(
                f"num_neg={num_neg} requires at least {num_neg + 1} unique destinations; "
                f"got {len(self.unique_destinations)}."
            )
    ...
    def sample(self, batch_destinations: np.ndarray):
        ...
        if not (batch_destinations.unsqueeze(1) == self.unique_destinations).any(dim=1).all():
            raise ValueError("Each positive destination must belong to the sampling pool.")
