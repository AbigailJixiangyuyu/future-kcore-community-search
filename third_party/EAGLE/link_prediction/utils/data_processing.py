...
def get_data_transductive(dataset_name, use_validation=False, data_path=None,
                         split_times=None):
    ...
    if data_path is None:
        directory = "benchtemp_datasets" if dataset_name in other_datasets else "data"
        data_path = Path(__file__).resolve().parents[2] / directory / dataset_name / f"ml_{dataset_name}.csv"
    graph_df = pd.read_csv(data_path)
    if "i" not in graph_df and "v" in graph_df:
        graph_df = graph_df.rename(columns={"v": "i"})
    if not {"u", "i", "ts"}.issubset(graph_df.columns):
        raise ValueError("CSV must contain u,v,ts (or u,i,ts).")
    if len(graph_df) < 3 or graph_df[["u", "i", "ts"]].isnull().any().any():
        raise ValueError("CSV must contain at least three events and no missing endpoints/times.")
    graph_df["ts"] = pd.to_numeric(graph_df["ts"], errors="raise")
    if not np.isfinite(graph_df.ts.to_numpy()).all():
        raise ValueError("Timestamps must be finite.")
    graph_df = graph_df.sort_values("ts", kind="stable").reset_index(drop=True)
    # Dense shared IDs keep TPPR arrays bounded even for sparse custom node IDs.
    endpoints = pd.concat([graph_df.u, graph_df.i], ignore_index=True)
    codes, _ = pd.factorize(endpoints, sort=False)
    graph_df["u"] = codes[:len(graph_df)]
    graph_df["i"] = codes[len(graph_df):]
    if "idx" not in graph_df:
        graph_df["idx"] = np.arange(len(graph_df), dtype=np.int64)
    if "label" not in graph_df:
        graph_df["label"] = 1
    ...
    val_time, test_time = (list(np.quantile(graph_df.ts, [0.70, 0.85]))
                           if split_times is None else split_times)
    ...
    if not all(mask.any() for mask in (train_mask, val_mask, test_mask)):
        raise ValueError("Timestamp quantiles produced an empty train/validation/test split.")
