...
if LooseVersion(th.__version__) < LooseVersion("1.9.0"):
    if (os.environ.get("SWIFT_ALLOW_OLD_TORCH") != "1" or
            LooseVersion(th.__version__) < LooseVersion("1.7.1")):
        raise RuntimeError("DGL requires PyTorch >= 1.9.0; SWIFT local testing "
                           "on 1.7.1 requires SWIFT_ALLOW_OLD_TORCH=1")
